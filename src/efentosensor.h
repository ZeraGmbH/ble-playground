...
public:
    ...
    //bool            checkMsgValid();
    unsigned char   checkMsgType();             // FW6
    void            decodeMeasureValues();      // FW6
    void            decodeAdvertiseValues();    // FW6
    unsigned long   getLastMeasureTS(); // FW6
    ...
    */
    ...
    //static constexpr unsigned char m_frameTypeStatus = 2;
    ...
    static constexpr unsigned long m_errorHumidExceedRange = 1<<3;
    static constexpr unsigned long m_errorAirPressExceedRange = 1<<4;
    static constexpr unsigned long m_errorHumidValueNegtive = 1<<5;
    static constexpr unsigned long m_errorAirPressValueNegtive = 1<<6;



    static constexpr unsigned long m_warningLowBattery = 1<<0;  //FW6
    static constexpr unsigned long m_warningMeasuremPeriBaseFalse = 1<<1;  //FW6
    static constexpr unsigned long m_warningMeasuremPeriFactFalse = 1<<2;  //FW6
    static constexpr unsigned long m_warningNoCalibDateSet = 1<<3;  //FW6
...
private:
    ...
    bool            m_battLevelOK;      // FW 6.x.x
    bool            m_encryptionEnable;  // FW 6.x.x
    unsigned long   m_measurementTs;    // FW6
    unsigned int    m_measurementPeriodBase;    // FW6

    //bool           m_storageErrorActive;
    float           m_temperaturInC;    // FW6
    float           m_temperaturInF;    // FW6
    unsigned char   m_humidity;         // FW6
    float           m_airPressure;      // FW6
    ...
    unsigned int   m_calibrationDate;   //FW6
